...
Public Sub ExecuteMemoryMapList(SQL As SQL, Query As String, StringArgs() As String, Limit As Int) As List
	Dim cur As Cursor
	If StringArgs <> Null Then 
		cur = SQL.ExecQuery2(Query, StringArgs)
	Else
		cur = SQL.ExecQuery(Query)
	End If
	Log("ExecuteMemoryMapList: " & Query)
	Dim table As List
	table.Initialize	
	If Limit > 0 Then Limit = Min(Limit, cur.RowCount) Else Limit = cur.RowCount
	For row = 0 To Limit - 1	
...
Sub GetDBVersion (SQL As SQL) As Int
	...
	Else
		...
		CreateTable(SQL, "DBVersion", m, "version", False)
...
Sub DeleteRecordID(SQL As SQL, id As Int)
	SQL.BeginTransaction
	'SQL.ExecNonQuery2("DELETE FROM navigation WHERE key = ?", Array As Object(id))
	SQL.ExecNonQuery("DELETE FROM navigation WHERE key = " & id)
End Sub